...
class MetasploitModule < Msf::Auxiliary
  ...
  def find_auth_uri
    if datastore['AUTH_URI'] && !datastore['AUTH_URI'].empty?
  ...
  def get_login_hidden(res)
    ...
    form = res.body.split(%r{<form action=([^>]+) method="post" id="form-login">(.*)</form>}mi)

    if form.length == 1 # is not Joomla 2.5
      vprint_status("#{target_url} - Testing Form Joomla 3.0 Form...")
      form = res.body.split(%r{<form action=([^>]+) method="post" id="form-login" class="form-inline">(.*)</form>}mi)
    end

    if form.length == 1 # is not Joomla 3
      vprint_error("#{target_url} - Last chance to find a login form...")
      form = res.body.split(%r{<form id="login-form" action=([^>]+)>(.*)</form>}mi)
    end

    begin
      input_hidden = form[2].split(%r{<input type="hidden"([^>]+)/>}mi)
