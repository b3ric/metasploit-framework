      ...
      if res && res.body.include?('<td><h1>Application Trace</h1></td>')
        ...
        if datastore['TRACE_DETAILS']
          ...
          result.each do |u|
            ...
            reg_info = [
              %r{<td>UserId</td><td>(\w+.*)</td>},
              %r{<td>Password</td><td>(\w+.*)</td>},
              %r{<td>APPL_PHYSICAL_PATH</td><td>(\w+.*)</td>},
              %r{<td>AspFilterSessionId</td><td>(\w+.*)</td>},
              %r{<td>Via</td><td>(\w+.*)</td>}, %r{<td>LOCAL_ADDR</td><td>(\w+.*)</td>},
              %r{<td>ALL_RAW</td><td>((.+\n)+)</td>}
            ]
