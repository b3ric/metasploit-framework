...
class MetasploitModule < Msf::Auxiliary
  ...
  def fingerprint(response)
    if response.headers.key?('Server')
      if (response.headers['Server'] =~ (/IIS/) || response.headers['Server'] =~ (/\(Windows/))
        os = "Windows (#{response.headers['Server']})"
      elsif (response.headers['Server'] =~ %r{Apache/})
    ...
    title = 'Not Found'
    response.body.gsub!(/[\r\n]/, '')
    if (response.body =~ %r{<title.*/?>(.+)</title/?>}i)
    ...
    if (response.body =~ %r{>\s*Version:\s*(.*)</strong><br\s/})
  ...
  def run_host(ip)
    ...
    if ((trav == '') || datastore['FINGERPINT'])
      ...
      if (res.code.to_i == 200)
        ...
        elsif (res.body =~ (/Adobe/) && res.body =~ (/ColdFusion/))
    ...
    urls = [
      '/CFIDE/administrator/enter.cfm', '/CFIDE/wizards/common/_logintowizard.cfm', '/CFIDE/administrator/archives/index.cfm',
    ...
    urls.each do |url|
      ...
      if res.nil?
        ...
        if res.body.match(%r{<title>(.*)</title>}im)
