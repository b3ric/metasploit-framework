...
class MetasploitModule < Msf::Auxiliary
  ...
  def login
    ...
    if res && (res.code == 200) && res.body.to_s =~ (%r{self.location="\.\./cgi/url_redirect\.cgi}) && res.get_cookies =~ (/(SID=[a-z]+)/)
  ...
  def read_file(file, session)
    ...
    if res && (res.code == 200) && res.headers['Content-type'].to_s =~ (%r{text/html}) && res.headers['Pragma'].nil?
