...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(target_host)
    if (datastore['RPORT'].to_i == 80) || (datastore['RPORT'].to_i == 443)
      port = ''
    else
    ...
    if res.match(%r{HTTP/1.1 200 OK})
      if res.match(/Server: (.*)/)
      ...
      if (fpversion = res.match(/FPVersion="(.*)"/))
        ...
        if (fpauthor = res.match(/FPAuthorScriptUrl="([^"]*)/))
          ...
          opts[:port] = datastore['RPORT'] if !port.empty?
          report_note(opts)
        end
  ...
  def check_account(info, fpversion, target_host)
    ...
    if (res && res.match(%r{^HTTP/1\.[01]\s+([^\s]+)\s+(.*)}))
