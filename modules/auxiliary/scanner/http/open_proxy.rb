...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(target_host)
    ...
    if datastore['VERIFYCONNECT']
      ...
      check_url = check_url.gsub(%r{[http://|https://]}, '')
      if check_url !~ /:443$/
        check_url += ':443'
      end
    else
      ...
      check_url = check_url.gsub(%r{https://}, '')
      if check_url !~ %r{^http://}i
  ...
  def verify_target(target_host, target_port, target_method, check_url, target_proxy_headers)
    ...
    begin
      ...
      target_proxy_headers.each do |proxy_header|
        next unless res.headers.to_s.match(/#{proxy_header}: (.*)/)
      ...
      elsif valid_codes.include?(res.code.to_s) && (res.headers.include?(datastore['VALIDPATTERN']) || res.body.include?(datastore['VALIDPATTERN']))
