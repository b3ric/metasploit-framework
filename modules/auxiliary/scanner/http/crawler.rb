...
=end
  ...
  def crawler_process_page(t, page, cnt)
    ...
    if page.error
      print_error("Error accessing page #{page.error}")
      elog(page.error)
    end
    case page.code
    when 301, 302
    ...
    return if ![302, 301, 200, 500, 401, 403, 404].include?(page.code)
    ...
    if page.doc
      page.doc.css('form').each do |f|
        ...
        if f['action'] && !f['action'].strip.empty?
          ...
          if (action[0, 1] != '/') && action !~ (%r{://})
            # Extract the base href first
            base = target.path.gsub(%r{(.*/)[^/]+$}, '\\1')
            page.doc.css('base').each do |bref|
              if bref['href']
                base = bref['href']
              end
            end
            action = (base + '/').sub(%r{//$}, '/') + action
          end
        ...
        next unless target.to_s !~ get_link_filter
  ...
  def form_from_url(website, url)
    ...
    # ex: /customers/343
    if !(form[:path]) && url.path.to_s =~ (%r{(.*)/(\d+)$})
    ...
    if !(form[:path]) && url.path.to_s =~ (%r{(.*/[a-z0-9A-Z]{3,256}\.[a-z0-9A-Z]{2,8})(/.*)})
      path_base = ::Regexp.last_match(1)
      path_info = ::Regexp.last_match(2)
      form[:method] = 'PATH'
      form[:path] = path_base
